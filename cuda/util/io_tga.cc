...
int saveTGAImage(float* floatdata, const char* filename, unsigned int resx, unsigned int resy, unsigned int numchannels) {
  unsigned char* data;
  struct tga_header {
      unsigned char id_len, pal_type, img_type;
      unsigned char pal_startH, pal_startL, pal_lenH, pal_lenL;
...
//returns the value (not the position) of the minmum and maximum
void getMinMax(float* data, float& min, float& max, unsigned int length){
    min = max = data[0];
...
void normalizeForTGAOutput(float* floatdata, unsigned int resx, unsigned int resy, unsigned int numchannels){
    unsigned int len = resx * resy * numchannels;
    float min, max;
    getMinMax(floatdata, min, max, len);
    printf("Normalizing texture length=%i: Min_old = %f, Max_old=%f\n",len, min,max);
    for(unsigned int i = 0; i < len; ++i){
        floatdata[i] = (floatdata[i] - min) / (max - min);
    }
}
